...
export default function CategorizedMenu({
    ...
    return (
        ...
        <>{
            menu ? (
                <div>
                    <div className="text-3xl mb-5 mt-6 text-black bg-white text-center font-medium">
                        {menu && menu.categoryDisplay}
                    </div>
                    <div className="flex lg:flex-row flex-col">
                        {/* mobile */}
                        <div className={`w-full 
                    ${menu?.category === "bouquet" ? "bg-[#FFA8B6]"
                                : menu?.category === "indoor" ? "bg-[#7ACD7C]"
                                    : menu?.category === "outdoor" ? "bg-[#66CC66]"
                                        : menu?.category === "accessory" ? "bg-[#ffb845]"
                                        : menu?.category === "orchid" ? "bg-[#DA70D6]"
                                    ...
                                    <div className={`grid md:grid-cols-4 grid-cols-2 h-full ${menu && menu.subMenuCategory.length > 4 ? `md:grid-rows-2 grid-rows-3` : menu && menu.subMenuCategory.length > 2 ? `md:grid-rows-1 grid-rows-2` : `grid-rows-1`}`}>
                                        {
                                            menu && menu.subMenuCategory.length > 0 ?
                                                menu.subMenuCategory
                                                    ...
                                                    .filter(x => x.platform === "both" && x.visibility === true)
                                                    .map((subMenu) =>
                                ...
                                : menu?.category === "indoor" ? "bg-[#7ACD7C]"
                                    : menu?.category === "outdoor" ? "bg-[#66CC66]"
                                        ...
                                        : menu?.category === "orchid" ? "bg-[#DA70D6]"
                                            ...
                                            : "bg-[#fa4d4d]"} 
                    m-3 h-auto lg:flex items-center justify-center hidden rounded-3xl`}>
